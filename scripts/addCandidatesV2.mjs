...
const cands=JSON.parse(fs.readFileSync(path.join(ROOT,'tmp','candidates_v2.json'),'utf8'));
const DONE=path.join(ROOT,'tmp','bulk_add_v2_done.json'); let done=new Set(); try{done=new Set(JSON.parse(fs.readFileSync(DONE,'utf8')));}catch{}
const ADDED=path.join(ROOT,'tmp','bulk_added_v2.json'); let added=[]; try{added=JSON.parse(fs.readFileSync(ADDED,'utf8'));}catch{}
