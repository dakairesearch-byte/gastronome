/**
 * Menu scraper v100 — definitive, stall-proof, resume-safe.
 *
 * Built from everything learned across v1 → v2.3:
 ...
 *
 * This version fixes every failure mode we've observed and introduces a
 * heartbeat + watchdog protocol so the run cannot die silently.
 *
 * ========================================================================
 * PIPELINE (per restaurant)
 * ========================================================================
 *   1. Skip if already has website / website-browser / website-v100 items.
 ...
 *      Menu, follows /menus/* anchors. ~10-25s per site.
 *   4. Persist items as source='website-v100', tag stage in price_raw.
 *   5. Log result to restaurant_menu_fetches.
 *
 * ========================================================================
 * STALL DEFENSES
 * ========================================================================
 *   - Per-operation timeouts wrap *every* awaited call: fetch(), goto(),
 *     newContext(), page.evaluate(), supabase.insert/upsert.
 *   - Per-restaurant hard ceiling (120s) races the whole pipeline.
 *   - Heartbeat file `tmp/v100-heartbeat.json` is rewritten every 2 seconds
 *     AND before every major transition. An external watchdog can poll it
 *     and force-kill if stale.
 *   - Chromium is recycled every `--browserRecycle` restaurants (default 20)
 *     to drain native-memory growth.
 *   - Graceful SIGTERM/SIGINT: flushes progress file, closes browser, exits
 *     with code 130 so the watchdog can resume from the progress file.
 *
 * ========================================================================
 * IDEMPOTENT RESUME
 * ========================================================================
 *   - tmp/v100-progress.json holds the per-ID status. On startup, any ID
 *     with status 'ok'|'rejected'|'no_menus' is skipped. 'errored' is
 *     retried. The watchdog can restart this process safely at any time.
 *   - IDs are chunked internally to <=40 per Supabase .in() call so the
 *     PostgREST URL never exceeds the 16KB header limit.
 *
 * ========================================================================
 * USAGE
 * ========================================================================
 *   npx tsx scripts/scrapeMenusV100.ts --ids=a,b,c
 *   npx tsx scripts/scrapeMenusV100.ts --idsFile=tmp/no-menu-priority-ids.txt
 ...
 *   --heartbeatMs=2000
 *   --heartbeatPath=tmp/v100-heartbeat.json
 *   --progressPath=tmp/v100-progress.json
 *   --reportPath=tmp/v100-report-<ts>.json
 *   --maxMenusPerSite=8
...
const CFG = {
  ...
  heartbeatPath: argStr('heartbeatPath', 'tmp/v100-heartbeat.json'),
  progressPath: argStr('progressPath', 'tmp/v100-progress.json'),
  reportPath: argStr('reportPath', `tmp/v100-report-${Date.now()}.json`),
...
const INPUT_IDS = loadIds()
if (!INPUT_IDS.length) {
  console.error('v100: either --ids=csv or --idsFile=path required')
  process.exit(1)
}
  ...
  for (let i = 0; i < ids.length; i += 40) {
    ...
    const r = await withTimeout(
      supabase.from('restaurant_menu_items')
        ...
        .in('source', ['website', 'website-browser', 'website-v100', 'website-vision']),
...
async function persistItems(id: string, items: Item[], sourceUrl: string, stage: 'fetch' | 'browser', q: MenuQuality): Promise<void> {
  ...
  await withTimeout(
    supabase.from('restaurant_menu_items').delete().eq('restaurant_id', id).eq('source', 'website-v100'),
    CFG.dbTimeoutMs, 'delete_prior_v100',
  )
  ...
  await withTimeout(
    supabase.from('restaurant_menu_fetches').insert({
      ...
      error_message: `v100 ${stage} food_ratio=${q.food_ratio.toFixed(2)} drink_ratio=${q.drink_ratio.toFixed(2)} verdict=${q.verdict}`,
    }),
...
async function main() {
  console.log(`[v100] pid=${process.pid} stages=${CFG.stages} total_ids=${INPUT_IDS.length}`)
  flushHeartbeat()
  ...
  console.log(`[v100] todo=${todo.length} already-persisted=${existing.size} input=${INPUT_IDS.length}`)
